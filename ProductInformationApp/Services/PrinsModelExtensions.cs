...
public static class PrinsModelExtensions
{
    ...
    public static IEnumerable<(string, object)> GetHints(this Product product)
    {
        ...
        if (product.ProductCertifications != null) {
            ...
            //}
            yield return (nameof(product.ProductCertifications), product.ProductCertifications);
        }

        // # Lagerhinweise
        if (product.StorageInstruction != null) {
            yield return (nameof(product.StorageInstruction), product.StorageInstruction.Value);
        }

        // # Zubereitungsempfehlung
        if (product.PreparationInfo != null) {
            yield return (nameof(product.PreparationInfo), product.PreparationInfo.Value);
        }

        // # Kostformen
        if (product.DietTypes != null) {
            yield return (nameof(product.DietTypes), product.DietTypes);
        }
    }
    ...
    {
        //if (product.NutrientAnnotation != null) {
        //    yield return (nameof(product.NutrientAnnotation), product.NutrientAnnotation.Value, "");
        //}
    ...
    public static IEnumerable<(string, string, int)> GetAlergens(this Product product)
    {
        ...
        alergens = product.Allergens2.Select(a => (a.ShortName ?? a.Name, a.Text, a.Flag));
        foreach (var n in alergens) {
            yield return n;
        }
    }
